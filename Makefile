...
SUBMAKE := $(MAKE) C_INCLUDE_PATH="$(C_INCLUDE_PATH)" CPLUS_INCLUDE_PATH="$(CPLUS_INCLUDE_PATH)" LD_LIBRARY_PATH="$(LD_LIBRARY_PATH)" TARGET_FLAGS="$(TARGET_FLAGS)" CFLAGS="$(CFLAGS)" LDFLAGS="$(LDFLAGS)" NO_GETTEXT=1 NO_DARWIN_PORTS=1 NO_HOMEBREW=1 prefix=$(GIT_PREFIX) DESTDIR=$(DESTDIR)
...
$(BUILD_DIR)/git-$(VERSION)/Makefile: build/git-$(VERSION).tar.gz
	...
	for p in patches/*.patch; do git apply --stat --apply --directory=$(BUILD_DIR)/git-$(VERSION) "$$p" || { echo "patch $$p failed"; exit 1; } ; done
	touch $@
